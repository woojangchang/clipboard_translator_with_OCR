...
try :
    with open('hotkeys.txt', 'r', encoding='utf8') as f:
        fh = f.read()
    ...
    hotkeylists = [{None}, {'Alt', 'D'}, {None}, {'F', 'Alt'}, {'Alt', 'C'}, {'Alt', 'T'}, {'F4'}]
...
# # OCR 언어 목록
# ocr_dic = {'한영일':'kor+eng+jpn+jpn_vert', '한+영':'kor+eng', '한국어':'kor', '영어':'eng', '일본어(가로)':'jpn', '일본어(세로)':'jpn_vert', '일본어(자동)':'jpn+jpn_vert'}
# v0 = ocr_dic.values()
# k0 = ocr_dic.keys()
# ocr_dic_change = {}
...
root.title("Clipboard Translator (Google)")
...
def titlechange(event=None):
    if translate_var.get() == 0 :
        ...
        root.title("Clipboard Translator (Papago)")
...
# menu_ocr.add_checkbutton(label='줄바꿈 없애기', variable=ocr_line_var, command = noline)
...
def showinfo():
    ...
    label = ttk.Label(toplevel_frame, text='Clipboard Translator', font=font.Font(size=16, weight='bold')) 
    label.grid(row=0, column=3, padx=10)
    label2 = ttk.Label(toplevel_frame, text='Clipboard Translator v'+str(version)+'\nDate : '+date+'\nDeveloped by r24',
...
def showhelp(event=None):
    global toplevelx, frame_1_open, frame_3_open
    toplevelx = False
    ...
    frame_1_open = False
    def frame_1_button():
        global frame_1_open, frame_3_open
        if frame_1_open :
            frame_1_open = False
...
def manage_hotkeys():
    ...
    ttk.Label(frame1, text='플로팅 번역창 띄우기/닫기', font=font_body, justify='left').grid(row=8, column=0, sticky=W, padx=5)
    ...
    def return_hotkeys():
        ...
        for i, s in enumerate(hotkeylists) :
            if s == {None} :
                ...
                hotkeylists_use.append(hotkey)
                hotkeylists_regist.append(rehk)
        with open('hotkeys.txt', 'w', encoding='utf8') as f:
            for i in range(7):
...
# ocr_on = False                
def determinate():
    global texto, textn, text_clipboardo, text_clipboardn, lator_check, destlang_check, srclang_check
    try:
...
#     except :
#         text_output = r'C:\Program Files\Tesseract-OCR\tesseract.exe'+' 파일이 존재하지 않습니다.'
